...
NDN.  Our analysis reveals strong incompatibilities in at least two
...
NDN considers the main focus of networking (as we believe). In NDN,
...
are what NDN centers on. SSB, on the other hand, has no global
constructs, true to is decentralized point of view, and can be called
{\em name-centric} in order to contrast it from {\em name-centric}
NDN.
...
future a synthesis of the two approaches may emerge, but we have not
...
Instead of structured JSON data, the content can be a base64-encoded byte string of encrypted data (of structured JSON), together with a tag that signifies which encryption algorithm was used.
