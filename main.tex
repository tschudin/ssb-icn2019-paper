...
other posts and their replies, which form a directed acyclic graph.\footnote{We call "tangle" the resulting data structure in which writers facilitate the causal ordering by explicitly referring to the most recent messages in other logs they were aware of at the time of writing, regardless of whether the content was directly relevant to their own message.} Insertion into the dependency graph may or may not lead to having to rerun the sort on the whole graph of
...
% XXX
To this end we use the existing SSB peering protocol and map its
functionality to a NDN-style packet-level design/protocol/consideration.
...
consumer can subscribe to a feed for a limited number of packets, effectively
defining a subrange of a feed. As
...
that new events correctly extend the local copy of the feed.\footnote{
  ...
  of a feed.}
%
Lost packets are detected when the received sequence number does not
match the expected value. Different retransmission strategies can be
envisaged, starting from selective retransmission via a classic NDN
Interest, or go-back-N with a LLI packet for a range that starts at
the first missed packet. Less PIT state is needed for such a go-back-N
strategy (thanks to the feed concept where ranges compactely encode
multiple names), when compared to an emulated PUSH using a polling
strategy and sending multiple Interests in parallel. This is an
example of an optimization which becomes possible because the network
knows about the structure of feeds.  Investigating other
retransmission mechanisms deserves further research.

% YYY avoid to send WW interests for future data names
% YYY Compared to polling for multiple individual names in parallel
% YYY order to implement push in NDN), less PIT state is used and

We call this a controlled push for two reasons. First, consumers guard
...
classic pull. Second, SSB's cryptographically secured append-only logs
...
need to be cached to serve most consumers. The memory usage shall be
similar to the current PIT state.
...
log); consumers stay anonymous and don't need their own routable
prefix. Additional benefits exist due to the network being aware of
the log's semantic, for example being able to request in-network
retransmission when observing missing log entries, instead of having
...
\section{Future Work} % SSB's ``Work in Progress''}
\label{sec:wip}
