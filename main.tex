...
TODO: clean up terminology (feed/log/author/identity/id, server/device/node/peer/machine, message/data/entry, ``social graph'', subjectivity (subjective reader) as opposed to plurality (how to interpret messages))
...
map-reduce would be executed at run-time. Instead, almost each
application will build indexes and aggressively cache state that was
already aggregated. Should ever the indexes become corrupted
...
list of posts and being able to record a post's submissiontime, leads
...
position in the social graph which controls replication and
...
that eventual consistency is a ``good enough'' basis for real
...
\begin{itemize}
  ...
  \item adressing messages by a pair of author and sequence number, unlike hash-based addressing this allows cyclic references
\end{itemize}
...
TODO: That's not a good introduction...
...
TODO: Does this really contribute to the paper, or am I just showing off?
...
TODO this might be a good place for mentioning encryption?
