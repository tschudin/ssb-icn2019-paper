...
\subsection{Example SSB app: about}

``about'' = a simple directory app / each peer is represented as a set
of key-value bindings / the log entries capture the updates to these
bindings

\ \\
\centerline{simple fig here}
\ \\

the latest directory state consists in executing all update events
found in all (available) logs / this directory state is relevant to
users as well as the forwarding fabric (set of IDs to support). Note
well the ``subjective reader'' property: the content/view of a peer's
user directory is dependent on its position in the social graph.  Note
also that ``about log entries'' for {\em other} peers can be added to
one's log e.g. for defining (subjective) display names for other
users.
