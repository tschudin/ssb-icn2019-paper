...
\subsection{Profiles of selected SSB apps}

Multiple applications have been written by contributors and are used
daily by the community. We briefly present selected examples because they 
represent alternatives to well-known services and they illustrate both opportunities
and challenges of communication through replicated append-only logs.

\textit{Git-ssb} is an alternative to GitHub~\cite{github} that replicates git-based 
version-controlled code repositories through contributors logs. It provides an encoding 
of repositories in SSB logs, a bridge to interoperate with git repositories, and a web-based
viewer to browse repositories. The object 
model of Git~\cite{chacon2014pro}, based on immutable hash-referenced objects organized in a 
chain of commits, has a similar structure to SSB's logs, making the mapping 
natural. Hash objects are blobs and commits are messages in individual append-only logs. 
Other git operations, such as creating a repository, creating merge commits,
creating a branch, or requesting the merging of an alternative branch (\textit{pull-request}) 
to a core maintainer, are all SSB messages. This model
provides automatic distribution of code repositories and their updates. Many developers
can also update and perform operations the \textit{same} repository, as defined by its creation message, 
independently. Consensus on the "official" master branch and its latest commit is enforced through social coordination
because the developers of the community know and trust each other. Nonetheless, 
in case of concurrent updates to the same branch in the same repository, the git bridge will create multiple branches when 
a local git repository is updated. A user can then resolve the fork by merging the diverging branches and updating the repository.

(TODO: Refer to the Tangle discussion and make shorter).




% In fact,
%it was an inspiration for the design of the replicated append-only logs communication model. The commit history of git has richer semantics
%than SSB logs because it provides merging operations between different chains of commits.
%
%
% The hash-objects can directly be stored in blobs, the creation of a repository,
%the addition of commits, the creation of a branch, and the creation of a request for merging an alternative branch (\textit{pull-request})
%are all messages in developers' feeds.
%
%
%in a user feed.
%
%A repository, a commit, a reference (branch), 
%
%
%In fact, it was a direct inspiration for the design of SSB.
%
%hash-, tree-, and commit-objects
%can all easily 
%
%
%git-ssb (https://git.scuttlebot.io/%25n92DiQh7ietE%2BR%2BX%2FI403LQoyf2DtR3WQfCkDKlheQU%3D.sha256)
%-> Clarify how the conflict resolution relates to the Tangle explanations (https://github.com/noffle/git-ssb-intro#push-conflicts)
%
%chess (https://github.com/Happy0/ssb-chess)
%gatherings plugin (https://github.com/pietgeursen/patch-gatherings)
%ssb-drive (https://github.com/cn-uofbasel/ssbdrv/)
%scry (https://github.com/ssbc/patchbay-scry)
%follow
