...
The basic ideas behind SSB can be traced back to the nineties: Secure logging~\cite{TODO} and secure relative time-stamping~\cite{TODO}. The major innovation of SSB is to use these techniques for disseminating data through a gossip protocol in a network of untrusted peers.
% https://scholar.google.de/scholar?q=Cryptographic+Support+for+Secure+Logs+on+Untrusted+Machines&hl=en&as_sdt=0,5
% https://scholar.google.de/scholar?hl=en&as_sdt=0,5&q=How+to+TimeStamp+a+Digital+Document
% google scholar currently returns a 403 when trying to access the bibtex citation snippet...
...
This section outlines a few dimensions in which the SSB log format could be extended to provide useful additional features.
...
% - db layer: indexing
% - message type coordination? schemas?
%
% - (delegating) moderation
% - (delegating) forbidden blobs and messages (the latter assuming offchain-content)
% - onboarding
